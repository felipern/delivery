...
class Endpoints
{
    ...
    private function map($method)
    {
        if (!isset($this->endpoints['ENDPOINTS'])) {
            if (array_column($this->endpoints['INSTANCES'], $method)) {
                $this->endpoints = $this->endpoints['INSTANCES'];
                $this->options['api'] = 'INSTANCES';
            } else if (array_column($this->endpoints['MESSAGES'], $method)) {
                $this->endpoints = $this->endpoints['MESSAGES'];
                $this->options['api'] = 'MESSAGES';
            } else {
                throw new Exception("Nonexistent requested '$method' method");
            }
        }
